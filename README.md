# 🌐 Multi Landing Pages Project

This repository contains **7 responsive landing pages** built with **HTML, CSS, and a bit of JavaScript**.
Additionally, it includes a **modern image/content slider** implemented using **Swiper.js** for smooth and interactive transitions.

## 🚀 Features

* 7 different landing pages 
* Built using HTML5, CSS3, and vanilla JavaScript
* Swiper.js integration for responsive sliders
* Clean and organized code structure
* Easy to customize and extend

## 🛠️ Technologies Used

* HTML5 – Page structure
* CSS3 – Styling and responsiveness
* JavaScript (ES6) – Interactive features
* Swiper.js – Modern, touch-enabled slider

## 📂 Project Structure

project-folder
│
├── landing-page-1
├── landing-page-2
├── landing-page-3
├── landing-page-4
├── landing-page-5
├── landing-page-6
├── landing-page-7
│
├── swiper-slider
│   ├── index.html
│   ├── style.css
│   └── script.js
│
└── README.md

## 📸 Preview

Each landing page showcases a different design theme — from minimalistic business layouts to creative modern styles.
You can explore them individually by opening their respective HTML files in your browser.

## ⚙️ How to Use

1. Clone this repository:
   git clone [Repo](https://github.com/SultanAli41/Landing-Pages.git)
2. Open any landing page folder.
3. Run the HTML file in your browser (double-click or use Live Server in VS Code).
4. Explore the Swiper Slider by opening the `swiper-slider/index.html` file.

## 📦 Dependencies

The Swiper.js library is included via CDN in the project:

<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.css" />  
<script src="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.js"></script>  

## ✨ Future Enhancements

* Add dark/light mode support
* Include animations using GSAP or AOS
* Integrate contact forms using JS or Firebase

## 📄 License

This project is open-source and available under the MIT License.
Feel free to use, modify, and share it.

## 👨‍💻 Author

**Sultan Ali**
📧 [Email](mailto:Sultanalibalouch512@gmail.com)
💼 [Github](https://github.com/SultanAli41)

⭐ If you like this project, don’t forget to star the repository!
